...
import { POLL_INTERVAL_MS, RANGES, chartConfig, fmtHour, fmtDay, fmtTime } from "./constants";

function VehiclesAreaChart({
	data,
	xKey,
	xFormatter,
}: {
...
export default function Dashboard() {
	...
	if (!data) {
		return <div className="flex flex-1 items-center justify-center py-32 text-muted-foreground">Loading...</div>;
	}

	const chartData = range === "today" ? data.today : range === "7d" ? data.last7 : data.last30;

	const title =
		range === "today" ? "Vehicles Today" : range === "7d" ? "Vehicles Last 7 Days" : "Vehicles Last 30 Days";
	const subtitle = range === "today" ? "Vehicles per hour" : "Total vehicles per day";

	return (
		<div className="flex flex-1 flex-col gap-6 p-6">
			<div>
				<h1 className="text-2xl font-semibold">Traffic Dashboard</h1>
				<p className="text-sm text-muted-foreground">Vehicle monitoring at the intersection from IoT sensors</p>
			</div>
						...
						<TableBody>
							{data.logs.map((log) => (
								<TableRow key={log.id}>
									...
									<TableCell>
										{log.isCrowded ? <Badge>Busy</Badge> : <Badge variant="secondary">Quiet</Badge>}
									</TableCell>
								</TableRow>
							))}
						</TableBody>
